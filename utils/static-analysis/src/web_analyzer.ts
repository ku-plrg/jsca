...
const libs = ['lodash_4.15.0_min', 'lodash_4.17.19_min', 'lodash_4.17.21_min'];
...
function main() {
  ...
  getDirectDirs(dirPath)
    ...
    .forEach((dir) => {
      md += `## ${dir}\n`;
      let table = `| File | ${libs.join(' | ')} | Abs-T | Comp-T |\n|-|${Array(
        libs.length
      )
        .fill('-')
        .join('|')}|-|-|\n`;
      ...
      const { ms } = measureTime(`compare ${dir}`, () => {
        fileContents.forEach(({ filename, content }) => {
          try {
            ...
            const { value: fileHash, ms: hashMs } = measureTime(
              `abstract ${filename}`,
      ...
      md += `Time: ${ms}ms\n`;
      md += `|Abstraction Time|Compare Time|\n|-|-|\n|${totalAbsT}|${totalCompT}|\n`;
