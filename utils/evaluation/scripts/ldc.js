const detect = () => {
  ...
  var d41d8cd98f00b204e9800998ecf8427e_LibraryDetectorTests = {
    ...
    React: {
      ...
      test: function (win) {
        ...
        if (
          ...
          return {
            version: (win.React && win.React.version) || UNKNOWN_VERSION,
          };
        }
        return false;
      },
    ...
    'WP Rocket': {
      ...
      npm: null,
      test: async function (win) {
        const wpRocketLazyLoad =
          typeof RocketLazyLoadScripts !== 'undefined' ||
          typeof RocketPreloadLinksConfig !== 'undefined' ||
          typeof rocket_lazy !== 'undefined';
        const wpRocketRUCSS = !!document.querySelector('style#wpr-usedcss');
        const wpRocketComment =
          document.lastChild.nodeType === Node.COMMENT_NODE &&
          document.lastChild.textContent.includes('WP Rocket');
    ...
    NitroPack: {
      ...
      npm: null,
      test: async function (win) {
        const nitroPackGenerator = !!document.querySelector(
    ...
    'Chart.js': {
      ...
      test: function (win) {
        ...
        // detect bundled version
        if (
          [...win.document.querySelectorAll('canvas')].some((el) => el.$chartjs)
        ) {
          return { version: UNKNOWN_VERSION };
        }
        return false;
    ...
    FusionCharts: {
      ...
      test: function (win) {
        if (win.FusionCharts && Array.isArray(win.FusionCharts.version)) {
          return {
            version: win.FusionCharts.version.join('.') || UNKNOWN_VERSION,
          };
        }
        // detect bundled version
        if (
          [...win.document.querySelectorAll('.fusioncharts-container')].some(
            (el) => el.FusionCharts
          )
        ) {
          return { version: UNKNOWN_VERSION };
    ...
    Chartist: {
      ...
      test: function (win) {
        // detect bundled version
        if (
          [...win.document.querySelectorAll('svg')].some(
            (el) => el.attributes['xmlns:ct']
          )
        ) {
          return { version: UNKNOWN_VERSION };
        }
    ...
    SliderRevolution: {
      ...
      test: function (win) {
        ...
        } else if (win.jQuery && win.jQuery.fn && win.jQuery.fn.revolution) {
  ...
  async function wait() {
    ...
    console.log(JSON.stringify(libs.map((lib) => lib.name)));
  }
