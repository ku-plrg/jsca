const detect = () => {
  ...
  function hasCircle(v_path) {
    ...
    if (v_path.length < 1) return false;
    ...
    try {
      if (
        eval(`typeof (${cur_v}) != 'object' && typeof (${cur_v}) != 'function'`)
      )
        return false;
    } catch {
      return false;
    ...
    ancient_v = 'window';
    if (eval(`${ancient_v} == ${cur_v}`)) return true;
    for (let i = 0; i < v_path.length - 1; i += 1) {
      ancient_v += `["${v_path[i]}"]`;
      try {
        if (
          eval(
            `typeof (${ancient_v}) == 'object' || typeof (${ancient_v}) == 'function'`
          )
        )
          if (eval(`${ancient_v} == ${cur_v}`)) return true;
      } catch {
  ...
  function compare_Dict_V(_dict, v) {
    ...
    if (typeof v == 'string') {
      if (_dict['t'] == 4 && _dict['v'] == v.slice(0, 10).replace(/<|>/g, '_'))
        return true;
      else return false;
    }
    if (typeof v == 'object') {
  ...
  function sortScore(score_list) {
    for (let lib_info of score_list) {
      ...
      lib_info['files'].sort(
        (a, b) => b['bases'][0]['credit1'] - a['bases'][0]['credit1']
      );
  ...
  function filterList(lib_match_list) {
    ...
    for (let lib_info of lib_match_list) {
      if (
        lib_info['files'][0]['bases'][0]['matched'] > 1 &&
        lib_info['score'] > 50
      )
  ...
  let requests = [
    'https://raw.githubusercontent.com/aaronxyliu/PTdetector/refs/heads/main/data/blacklist.json',
    'https://raw.githubusercontent.com/aaronxyliu/PTdetector/refs/heads/main/data/pts.json',
    'https://raw.githubusercontent.com/aaronxyliu/PTdetector/refs/heads/main/data/DetectFile.json',
  ].map((url) => {
  ...
  Promise.all(requests).then((results) => {
    const [blacklist, pts, file_list] = results;
    ...
    const filtered_list = filterList(score_list);

    console.log(JSON.stringify(filtered_list.map((sl) => sl['lib name'])));
  });
};
