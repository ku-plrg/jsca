...
var cfgState;
var SKIP_BUILTIN = ['Math', 'String', 'Number', 'Object'];
var IGNORE_TYPEOF = true;
function visit(node) {
  switch (node.type) {
    ...
    case 'ForInStatement':
      ...
      if (node.right.type === 'SequenceExpression') {
        const last2 = node.right.expressions.pop();
        if (!last2) throw new Error('Sequence expression is empty');
    ...
    case 'LogicalExpression':
      addCond(node.left);
      if (node.operator === '&&') {
        const thenprev2 = condVisitor(node.right, true);
        const elseprev2 = cfgState.prevIds.filter(([, context]) => !context);
        cfgState.prevIds = thenprev2.concat(elseprev2);
      } else {
        const elseprev2 = condVisitor(node.right, false);
        const thenprev2 = cfgState.prevIds.filter(([, context]) => context);
        cfgState.prevIds = thenprev2.concat(elseprev2);
      }
      break;
    case 'MemberExpression':
...
function extractCFG(node) {
  const start = { id: 0, type: 'start' };
  cfgState = {
    currentId: 1,
    prevIds: [[start]],
...
function condVisitor(node, context) {
...
function endLoop(loop, id, prevIds) {
...
function insertSequence(value, type) {
  if (
...
function cfgToDot(nodes) {
  ...
  `;
      ...
      case 'condition':
        if (node.then !== void 0) {
          dotString += `  node${newId} -> node${node.then + 2} [label="true"];
  `;
        }
        if (node.else !== void 0) {
          dotString += `  node${newId} -> node${node.else + 2} [label="false"];
  `;
        }
        break;
      case 'block':
...
async function generatePNG(dotContent, outputPath) {
  try {
...
function getId(node) {
  ...
  simple(node, {
    CallExpression(node2) {
      if (
        node2.callee.type === 'Identifier' &&
        node2.callee.name === 'Symbol' &&
        node2.arguments.length > 0 &&
        node2.arguments[0].type === 'Literal' &&
        typeof node2.arguments[0].value === 'string'
      ) {
        const symbolValue = node2.arguments[0].value;
        if (value === void 0 && symbolValue.startsWith('JSCA_')) {
          value = symbolValue;
        }
      }
    },
    TemplateLiteral(node2) {
      const templateValue = node2.quasis[0]?.value.raw;
      if (value === void 0 && templateValue.startsWith('JSCA_')) {
        value = templateValue;
      }
    },
    Literal(node2) {
      if (
        value === void 0 &&
        typeof node2.value === 'string' &&
        node2.value.startsWith('JSCA_')
      ) {
        value = node2.value;
      }
    },
...
function extractFunctions(code) {
  ...
  try {
    ast = parse3(code, {
      ...
      // Support the ES6
      sourceType: 'script',
      // Treat the code as a script (not a module)
      locations: true,
...
var function_extractor_default = extractFunctions;

// src/utils/cfg_to_hash.ts
import { createHash } from 'crypto';
function toHash(value) {
  return createHash('sha256').update(value).digest('hex');
}
function hashNode(node, state, hashMap, visited = /* @__PURE__ */ new Set()) {
...
var cfg_to_hash_default = convertHash;
...
function measureTime(label, fn) {
  ...
  if (label) console.log(`${label} took ${seconds}s ${nanoseconds / 1e6}ms`);
  const ms = seconds * 1e3 + nanoseconds / 1e6;
  return { value: result, ms };
}
var timer_default = measureTime;

// src/a.ts
function getHash(raw, logStr) {
  const { value: functions } = timer_default(
    logStr && `extractFunctions in ${logStr}`,
    () => function_extractor_default(raw)
  );
  const { value: cfgs } = timer_default(logStr && `makeCFG in ${logStr}`, () =>
    cfg_default(functions)
  );
  const { value: hash } = timer_default(logStr && `hashCFG in ${logStr}`, () =>
